...
// Módulo de seguimiento disciplinario — gestionado por el Gestor del grupo.
...
if (!hasRole(['Gestor','Coordinador','Administrador'])) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !$soloLectura) {
    ...
    if ($form === 'hecho') {
        ...
        if (!$data['aprendiz_id'] || !$data['tipo_hecho'] || !$data['descripcion']) {
            $err = 'Complete aprendiz, tipo de hecho y descripción.';
        } else {
            if ($editId) {
                $stmt = $db->prepare("UPDATE disc_hechos SET aprendiz_id=?,ficha_id=?,fecha_hecho=?,tipo_hecho=?,descripcion=?,lugar=?,testigos=?,estado=? WHERE id=?");
                $stmt->execute([$data['aprendiz_id'],$data['ficha_id'],$data['fecha_hecho'],$data['tipo_hecho'],$data['descripcion'],$data['lugar'],$data['testigos'],$data['estado'],$editId]);
                $msg = 'Hecho actualizado.';
            } else {
                $stmt = $db->prepare("INSERT INTO disc_hechos (aprendiz_id,ficha_id,gestor_id,fecha_hecho,tipo_hecho,descripcion,lugar,testigos,estado) VALUES(?,?,?,?,?,?,?,?,?)");
                $stmt->execute([$data['aprendiz_id'],$data['ficha_id'],$data['gestor_id'],$data['fecha_hecho'],$data['tipo_hecho'],$data['descripcion'],$data['lugar'],$data['testigos'],$data['estado']]);
    ...
    if ($form === 'atencion') {
        ...
        $data = [
            ...
            'tipo_atencion'     => $_POST['tipo_atencion'] ?? 'Llamado de atención verbal',
        ...
        if (!$data['hecho_id'] || !$data['descripcion']) {
            $err = 'Seleccione el hecho y describa la atención.';
        } else {
            ...
            if (!$err) {
                if ($editId) {
                    ...
                    $db->prepare("UPDATE disc_hechos SET estado='En atención' WHERE id=? AND estado='Abierto'")->execute([$hechoId]);
                    // Si hay compromiso → estado Comprometido
                    if ($data['compromisos']) {
                        $db->prepare("UPDATE disc_hechos SET estado='Comprometido' WHERE id=?")->execute([$hechoId]);
                    }
                    $msg = 'Atención registrada.';
                }
                $tab = 'atenciones';
            }
        }
    ...
    if ($form === 'seguimiento') {
        ...
        if (!$hechoId || !$obs) {
            $err = 'Seleccione el hecho y escriba la observación.';
        } else {
            ...
            // Actualizar estado del hecho según resultado
            $estadoMap = ['Cumpliendo'=>'Comprometido','Reincidencia'=>'Abierto','Cerrado'=>'Cerrado'];
            $db->prepare("UPDATE disc_hechos SET estado=? WHERE id=?")->execute([$estadoMap[$resultado] ?? 'En atención', $hechoId]);
    ...
    if ($form === 'comite') {
        ...
        if (!$hechoId || !$motivo) {
            $err = 'Indique el motivo de remisión a comité.';
        } else {
            $db->prepare("UPDATE disc_hechos SET estado='Remitido a comité',remitir_comite=1 WHERE id=?")->execute([$hechoId]);
            ...
            $msg = 'Aprendiz remitido a comité disciplinario.';
            $tab = 'hechos';
        }
    }
}
...
$statsDisc = [
    ...
    'en_atencion' => count(array_filter($hechos, fn($h) => in_array($h['estado'], ['En atención','Comprometido']))),
    'reincidentes'=> count(array_filter($hechos, fn($h) => $h['reincidencias'] > 0)),
    'comite'      => count(array_filter($hechos, fn($h) => $h['estado'] === 'Remitido a comité')),
...
<div class="page-header">
    <div>
        ...
        <div class="page-subtitle">
            ...
            <?= $soloLectura ? ' — <span style="color:var(--naranja)">Vista de solo lectura</span>' : '' ?>
        </div>
    </div>
    <?php if (!$soloLectura): ?>
    <div style="display:flex;gap:8px;flex-wrap:wrap">
        <button class="btn btn-primary" onclick="openModal('modalHecho')">+ Registrar hecho</button>
        <button class="btn btn-secondary" onclick="openModal('modalAtencion')">+ Registrar atención</button>
    </div>
    <?php endif; ?>
</div>
...
<div class="stats-grid" style="margin-bottom:20px">
    <div class="stat-card warning"><span class="stat-icon">⚠</span><div class="stat-value"><?= $statsDisc['abiertos'] ?></div><div class="stat-label">Abiertos</div></div>
    <div class="stat-card info">   <span class="stat-icon">◐</span><div class="stat-value"><?= $statsDisc['en_atencion'] ?></div><div class="stat-label">En atención</div></div>
    <div class="stat-card danger"> <span class="stat-icon">↺</span><div class="stat-value"><?= $statsDisc['reincidentes'] ?></div><div class="stat-label">Reincidentes</div></div>
    <div class="stat-card danger"> <span class="stat-icon">◑</span><div class="stat-value"><?= $statsDisc['comite'] ?></div><div class="stat-label">En comité</div></div>
    <div class="stat-card">        <span class="stat-icon">✓</span><div class="stat-value"><?= $statsDisc['cerrados'] ?></div><div class="stat-label">Cerrados</div></div>
</div>

<!-- TABS -->
<div class="disc-tabs">
    <button class="disc-tab <?= $tab==='hechos'      ? 'active':'' ?>" onclick="switchTab('hechos')">⚠ Hechos (<?= count($hechos) ?>)</button>
    <button class="disc-tab <?= $tab==='atenciones'  ? 'active':'' ?>" onclick="switchTab('atenciones')">📋 Atenciones (<?= count($atenciones) ?>)</button>
    <button class="disc-tab <?= $tab==='seguimiento' ? 'active':'' ?>" onclick="switchTab('seguimiento')">◎ Seguimiento (<?= count($seguimientos) ?>)</button>
</div>

<!-- ════ TAB HECHOS ════ -->
<div class="disc-panel <?= $tab==='hechos' ? 'active':'' ?>" id="tab-hechos">
<?php if (empty($hechos)): ?>
    <div class="table-card"><div class="empty-state" style="padding:60px 20px"><div class="icon">📋</div><p>No hay hechos disciplinarios registrados.</p></div></div>
<?php else: ?>
            ...
            <table>
                ...
                <?php foreach ($hechos as $h):
                    $estCls = match($h['estado']) {
                        ...
                        'En atención'       => 'estado-atencion',
                        'Comprometido'      => 'estado-comprometido',
                        'Cerrado'           => 'estado-cerrado',
                        'Remitido a comité' => 'estado-comite',
                        default             => ''
                    };
                ?>
                <tr>
                    ...
                    <td style="font-size:11px;max-width:160px"><?= sanitize($h['tipo_hecho']) ?></td>
                    <td><span class="badge <?= $estCls ?>"><?= sanitize($h['estado']) ?></span></td>
                    ...
                    <td style="text-align:center;color:<?= $h['reincidencias']>0?'var(--rojo)':'inherit' ?>;font-weight:<?= $h['reincidencias']>0?'700':'400' ?>">
                        <?= $h['reincidencias'] > 0 ? '⚠ '.$h['reincidencias'] : '—' ?>
                    </td>
                    <td style="white-space:nowrap">
                        <?php if (!$soloLectura): ?>
                        <button onclick='editarHecho(<?= json_encode($h, JSON_HEX_TAG|JSON_HEX_APOS|JSON_HEX_QUOT) ?>)' class="btn btn-sm btn-secondary" title="Editar">✎</button>
                        <button onclick='prepararAtencion(<?= $h['id'] ?>,<?= $h['aprendiz_id'] ?>,"<?= sanitize($h['aprendiz_nombre']) ?>")' class="btn btn-sm btn-primary" title="Registrar atención">+ Atención</button>
                        <?php if ($h['remitir_comite'] || $h['reincidencias'] > 0): ?>
                        <button onclick='prepararComite(<?= $h['id'] ?>,<?= $h['aprendiz_id'] ?>,"<?= sanitize($h['aprendiz_nombre']) ?>")' class="btn btn-sm btn-danger" title="Remitir a comité">→ Comité</button>
                        <?php endif; ?>
                        <?php endif; ?>
                    </td>
                </tr>
                ...
                <tr style="background:var(--gris-bg)">
                    <td colspan="8" style="padding:4px 14px 10px 28px;font-size:11px;color:var(--gris-text)">
                        <strong>📝 Descripción:</strong> <?= sanitize(substr($h['descripcion'],0,200)) ?>
...
<?php if (empty($atenciones)): ?>
    <div class="table-card"><div class="empty-state" style="padding:60px 20px"><div class="icon">📋</div><p>No hay atenciones registradas.</p></div></div>
<?php else: ?>
    <div class="table-card">
        <div class="table-card-header"><div class="table-card-title">Atenciones y descargos</div></div>
        <div class="table-wrapper">
            <table>
                <thead><tr><th>Aprendiz</th><th>Tipo hecho</th><th>Tipo atención</th><th>Fecha</th><th>Resultado</th><th>Firma Inst.</th><th>Firma Apr.</th><th>Soporte</th></tr></thead>
                <tbody>
                ...
                ?>
                <tr>
                    <td><strong><?= sanitize($at['aprendiz_nombre']) ?></strong><br><small style="color:#888"><?= sanitize($at['instructor_nombre'] ?? '—') ?></small></td>
                    ...
                    <td style="text-align:center">
                        <?php if ($at['firma_instructor']): ?>
                            <button onclick="verFirma('<?= htmlspecialchars($at['firma_instructor']) ?>','Instructor')" class="btn btn-sm btn-secondary">✍ Ver</button>
                        <?php else: ?><span style="color:#ccc">—</span><?php endif; ?>
                    </td>
                    <td style="text-align:center">
                        <?php if ($at['firma_aprendiz']): ?>
                            <button onclick="verFirma('<?= htmlspecialchars($at['firma_aprendiz']) ?>','Aprendiz')" class="btn btn-sm btn-secondary">✍ Ver</button>
                        <?php else: ?><span style="color:#ccc;font-weight:700">⚠ Sin firma</span><?php endif; ?>
                    </td>
                    <td style="text-align:center">
                        <?php if ($at['archivo_ruta']): ?>
                            <a href="<?= BASE_URL.'/'.sanitize($at['archivo_ruta']) ?>" target="_blank" class="btn btn-sm btn-secondary">📄</a>
                        <?php else: ?>—<?php endif; ?>
                    </td>
                </tr>
                ...
                <tr style="background:var(--gris-bg)">
                    <td colspan="8" style="padding:4px 14px 10px 28px;font-size:11px;color:var(--gris-text)">
                        <?= $at['descripcion'] ? '<strong>Atención:</strong> '.sanitize(substr($at['descripcion'],0,160)) : '' ?>
...
<div class="disc-panel <?= $tab==='seguimiento' ? 'active':'' ?>" id="tab-seguimiento">
        ...
        <form method="POST" class="form-grid">
            ...
            <div class="form-group">
                ...
                <select name="hecho_id" id="seg_hecho" required onchange="cargarAprendizSeg(this)">
                    ...
                    <?php foreach ($hechos as $h): ?>
                    <?php if (!in_array($h['estado'], ['Cerrado','Remitido a comité'])): ?>
                    <option value="<?= $h['id'] ?>" data-aprendiz="<?= $h['aprendiz_id'] ?>">
                        <?= sanitize($h['aprendiz_nombre']) ?> — <?= sanitize($h['tipo_hecho']) ?> (<?= date('d/m/Y',strtotime($h['fecha_hecho'])) ?>)
                    </option>
                    <?php endif; ?>
                    <?php endforeach; ?>
                </select>
            ...
            <div class="form-group">
                ...
                <select name="resultado_seg">
                    ...
                    <option value="Reincidencia">Reincidencia ⚠️</option>
    ...
    <div class="table-card">
        ...
        <div class="table-wrapper">
            <table>
                <thead><tr><th>Fecha</th><th>Aprendiz</th><th>Hecho</th><th>Resultado</th><th>Observación</th></tr></thead>
                <tbody>
...
<div class="modal-overlay" id="modalHecho">
    <div class="modal" style="max-width:620px">
        <div class="modal-header">
            ...
            <button class="modal-close" onclick="closeModal('modalHecho')">✕</button>
        </div>
                ...
                <div class="form-grid">
                    <div class="form-group full">
                        ...
                        <select name="aprendiz_id" id="hecho_aprendiz" required>
                            ...
                            <option value="<?= $ap['id'] ?>" data-ficha="<?= $ap['ficha_id'] ?>">
                                <?= sanitize($ap['nombre']) ?> — Doc: <?= sanitize($ap['documento']) ?> / Ficha <?= sanitize($ap['numero_ficha']) ?>
                            </option>
                            <?php endforeach; ?>
                        </select>
                    ...
                    <div class="form-group">
                        ...
                        <select name="tipo_hecho" id="hecho_tipo" required>
                            ...
                            <option>Fraude académico</option>
                            <option>Otro</option>
                        </select>
                    </div>
                    <div class="form-group">
                        <label>Lugar</label>
                        <input type="text" name="lugar" id="hecho_lugar" placeholder="Salón, taller, etc.">
                    </div>
                    <div class="form-group">
                        <label>Estado</label>
...
<div class="modal-overlay" id="modalAtencion">
    <div class="modal" style="max-width:700px">
        <div class="modal-header">
            <div class="modal-title">Registrar atención / descargos / compromiso</div>
            <button class="modal-close" onclick="closeModal('modalAtencion')">✕</button>
        </div>
        <form method="POST" enctype="multipart/form-data" id="formAtencion">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <select name="tipo_atencion">
                            ...
                            <option>Remisión a bienestar</option>
                            <option>Otro</option>
                        </select>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group full">
                        ...
                        <textarea name="descripcion_atencion" required placeholder="¿Qué se hizo? Llamado de atención, citación, etc."></textarea>
                    </div>
                    <div class="form-group full">
                        <label>Descargos del aprendiz</label>
                        <textarea name="descargos_aprendiz" placeholder="Versión del aprendiz, justificaciones dadas..."></textarea>
                    </div>
                    <div class="form-group full">
            ...
            <div class="modal-footer">
                ...
                <button type="submit" class="btn btn-primary" onclick="capturarFirmas()">Guardar atención</button>
            </div>
        </form>
    </div>
</div>
...
<div class="modal-overlay" id="modalComiteDisc">
    <div class="modal" style="max-width:500px">
        <div class="modal-header">
            <div class="modal-title">Remitir a Comité Disciplinario</div>
            <button class="modal-close" onclick="closeModal('modalComiteDisc')">✕</button>
        </div>
        <form method="POST">
            ...
            <div class="modal-body">
                <div class="alert alert-error">⚠️ Esta acción remite el caso al comité. Use solo cuando se han agotado las instancias disciplinarias internas.</div>
                ...
                <div class="form-group">
                    ...
                    <textarea name="motivo_comite" required rows="4" placeholder="Detalle los hechos, atenciones realizadas, reincidencias y por qué se remite a comité..."></textarea>
                </div>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" onclick="closeModal('modalComiteDisc')">Cancelar</button>
                <button type="submit" class="btn btn-danger">Remitir a comité</button>
            </div>
        </form>
    </div>
</div>
...
<div class="modal-overlay" id="modalVerFirma">
    <div class="modal" style="max-width:400px">
        <div class="modal-header">
            ...
            <button class="modal-close" onclick="closeModal('modalVerFirma')">✕</button>
        </div>
...
function verFirma(src, quien) {
    document.getElementById('tituloVerFirma').textContent = 'Firma — ' + quien;
...
// Iniciar firmas cuando se abre el modal de atención
